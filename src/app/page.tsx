  ...
  return (
    ...
    >
      ...
      <circle
        ...
        className="text-[#e87537] transition-[stroke-dashoffset] duration-500"
      />
    </svg>
  );
}
...
function StepIndicator({ complete }: { complete: boolean }) {
  ...
  return (
    <span className="h-5 w-5 shrink-0 rounded-full border-[1.5px] border-border bg-white" />
  );
}
...
function GoFurtherCard({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex h-[120px] items-end justify-start bg-gradient-to-br p-4",
          accent
        )}
      >
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/90 shadow-sm">
          {icon}
        </div>
      </div>
  ...
  return (
    <div className="flex h-full w-full flex-col gap-2 p-4">
      {sources.map((s) => (
        <div
          key={s.label}
          className="flex items-center gap-2 rounded-lg bg-white/90 px-3 py-2 shadow-[0_1px_2px_rgba(0,0,0,0.04)]"
        >
